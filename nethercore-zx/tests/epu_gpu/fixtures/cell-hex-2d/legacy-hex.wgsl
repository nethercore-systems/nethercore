// Frozen pre-correction CELL and transitive helpers; rejected prototypes only.
// @epu_meta_begin
// opcode = 0x05
// name = CELL
// kind = bounds
...
fn frozen_cell_hash2(cell: vec2f, seed: f32) -> f32 {
    let p = cell + vec2f(seed * 17.3, seed * 31.7);
    return fract(sin(dot(p, vec2f(127.1, 311.7))) * 43758.5453123);
}

// 2D hash returning vec2 for Voronoi jitter
fn frozen_cell_hash2_vec2(cell: vec2f, seed: f32) -> vec2f {
    let p = cell + vec2f(seed * 17.3, seed * 31.7);
    let h = vec2f(
...
fn frozen_wrap_periodic_x(x: f32, period: f32) -> f32 {
    return fract(x / period) * period;
}

fn frozen_shortest_periodic_delta(x: f32, center: f32, period: f32) -> f32 {
    return x - center - round((x - center) / period) * period;
}

// Compute axis-cylinder UV from direction and alignment axis
fn frozen_cell_axis_cylinder_uv(dir: vec3f, axis: vec3f) -> vec2f {
...
fn frozen_cell_grid(uv: vec2f, density: f32) -> vec3f {
...
// Fractional circumference has unresolved partial-cell topology: leave it intact.
// Wrap the integer ID, not local geometry or fractional hash/seed inputs.
fn frozen_cell_offset_id_x(x: f32, density: f32) -> f32 {
    ...
    if abs(density - period) <= 0.00001 {
        let n = i32(period);
        return f32(((i32(x) % n) + n) % n);
    }
    return x;
}

// HEX variant: hexagonal cells with offset rows
fn frozen_cell_hex(uv: vec2f, density: f32) -> vec3f {
    let scaled = uv * density;
    ...
    let cell_id = vec2f(frozen_cell_offset_id_x(floor(offset_x), density), row);
...
fn frozen_cell_brick(uv: vec2f, density: f32, seed: f32) -> vec3f {
    let scaled = uv * vec2f(density, density * 0.5);

    let row = floor(scaled.y);
    let row_seed = frozen_cell_hash2(vec2f(row, density), seed + 11.0);
    var offset_x = scaled.x;
    if fract(row * 0.5) < 0.25 {
        offset_x += 0.5;
    }
    ...
    let cell_id = vec2f(frozen_cell_offset_id_x(floor(offset_x), density), row);
    let brick_seed = frozen_cell_hash2(cell_id, seed + 29.0);
...
fn frozen_cell_periodic_candidates(uv: vec2f, period: f32, seed: f32, jitter_scale: f32) -> vec3f {
    ...
    for (var dy = -2; dy <= 2; dy += 1) {
        for (var dx = -2; dx <= 2; dx += 1) {
            ...
            let jitter = frozen_cell_hash2_vec2(id, seed);
            let point = id + jitter * jitter_scale + vec2f(select(0.1, 0.0, jitter_scale == 1.0));
            let d = length(vec2f(frozen_shortest_periodic_delta(scaled.x, point.x, period), scaled.y - point.y));
            if d < first {
                second = first;
                first = d;
                owner = id;
    ...
    return vec3f(owner, (second - first) * 0.5);
}

fn frozen_cell_voronoi(uv: vec2f, density: f32, seed: f32) -> vec3f {
    // Preserve unresolved fractional topology and its existing seed API.
    if abs(density - round(density)) <= 0.00001 {
        return frozen_cell_periodic_candidates(uv, round(density), seed, 0.8);
    }
    let scaled = uv * density;
    ...
    for (var dy = -1.0; dy <= 1.0; dy += 1.0) {
        for (var dx = -1.0; dx <= 1.0; dx += 1.0) {
            ...
            let wrapped = vec2f(frozen_wrap_periodic_x(neighbor.x, density), neighbor.y);
            let jitter = frozen_cell_hash2_vec2(neighbor, seed);
            let point = wrapped + jitter * 0.8 + vec2f(0.1);
            let delta = vec2f(
                frozen_shortest_periodic_delta(scaled.x, point.x, density),
                scaled.y - point.y
            );
            let d = length(delta);
...
fn frozen_cell_shatter(uv: vec2f, density: f32, seed: f32) -> vec3f {
    if abs(density - round(density)) <= 0.00001 {
        return frozen_cell_periodic_candidates(uv, round(density), seed + 42.0, 1.0);
    }
    let scaled = uv * density;
    let base_cell = floor(scaled);
    ...
    for (var dy = -1.0; dy <= 1.0; dy += 1.0) {
        for (var dx = -1.0; dx <= 1.0; dx += 1.0) {
            let neighbor = base_cell + vec2f(dx, dy);
            let wrapped = vec2f(frozen_wrap_periodic_x(neighbor.x, density), neighbor.y);
            let jitter = frozen_cell_hash2_vec2(neighbor, seed + 42.0);
...
fn frozen_cell_radial(uv: vec2f, density: f32, axis: vec3f, dir: vec3f) -> vec3f {
    ...
    let ring_warp = (frozen_cell_hash2(vec2f(base_spoke_seed, base_ring_seed), density * 0.37) - 0.5)
        * mix(0.0, 0.16 / ring_count, ring_relief);
    let spoke_warp = (frozen_cell_hash2(vec2f(base_ring_seed, base_spoke_seed), density * 0.73) - 0.5)
        * (0.14 / spoke_count)
        * center_relief;
...
fn frozen_eval_cell(
    ...
    let uv = frozen_cell_axis_cylinder_uv(dir, axis);

    // Compute cell info based on variant: vec3(cell_id.xy, d_edge)
    var cell_info: vec3f;
    var brick_stress = 0.0;
    switch variant {
        case 0u: { cell_info = frozen_cell_grid(uv, density); }      // GRID
        case 1u: { cell_info = frozen_cell_hex(uv, density); }       // HEX
        case 2u: { cell_info = frozen_cell_voronoi(uv, density, seed); }  // VORONOI
        case 3u: { cell_info = frozen_cell_radial(uv, density, axis, dir); }  // RADIAL
        case 4u: { cell_info = frozen_cell_shatter(uv, density, seed); }  // SHATTER
        case 5u: { // BRICK
            cell_info = frozen_cell_brick(uv, density, seed);
        ...
        default: { cell_info = frozen_cell_grid(uv, density); }
    }
    ...
    let cell_value = frozen_cell_hash2(cell_id, seed);
    ...
    switch variant {
        ...
        case 5u: { // BRICK
            ...
            if abs(density - round(density)) <= 0.00001 {
                ...
                outline_x = frozen_shortest_periodic_delta(uv.x * density, cell_id.x, round(density));
            }
