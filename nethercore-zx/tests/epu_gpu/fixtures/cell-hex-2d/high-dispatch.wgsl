...
fn high_bounds(
    ...
    switch opcode {
        ...
        case OP_CELL: {
            return high_eval(dir, instr, base_regions);
        }
        case OP_PATCHES: {
...
fn high_layer(
    ...
    switch opcode {
        // ====================================================================
        // Feature opcodes (0x08+) - bounds handled by high_bounds
        // ====================================================================
...
fn high_scene(dir: vec3f, layers: array<vec4u, 8>) -> vec3f {
    ...
    for (var i = 0u; i < 8u; i++) {
        ...
        if opcode < OP_FEATURE_MIN {
            ...
            let result = high_bounds(dir, instr, opcode, bounds_dir, regions);
            regions = result.regions;
            radiance = apply_blend(radiance, result.sample, blend);
        } else {
            radiance = apply_blend(radiance, high_layer(dir, instr, bounds_dir, regions), blend);
        }
    }
    return radiance;
}
