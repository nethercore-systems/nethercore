...
@compute @workgroup_size(1) fn probe(@builtin(global_invocation_id) p:vec3u){
    ...
    var old=site_before_eval_cell(dir,i,RegionWeights(.2,.3,.5));
    ...
    let is_site=variant==2u || variant==4u;
    var info=vec3f(0.);var old_info=vec3f(0.);
    if variant==2u {info=cell_voronoi(uv,density,f32(seed));old_info=site_before_cell_voronoi(uv,density,f32(seed));}
    if variant==4u {info=cell_shatter(uv,density,f32(seed));old_info=site_before_cell_shatter(uv,density,f32(seed));}
