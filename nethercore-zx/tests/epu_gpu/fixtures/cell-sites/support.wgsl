...
@compute @workgroup_size(1) fn probe(@builtin(global_invocation_id) p:vec3u){
    let row=p.y%144u;let variant=array<u32,2>(2u,4u)[p.y/432u];
    ...
    let original=support_info(support_dir(vec2f(lo,v),axis),i);
    let end=support_info(support_dir(vec2f(hi,v),axis),i);
    if all(original.xy==end.xy) {textureStore(result,p.xy,vec4f(0.));return;}
    for(var k=0u;k<24u;k+=1u){
        let mid=(lo+hi)*.5;let info=support_info(support_dir(vec2f(mid,v),axis),i);
    ...
    for(var scale=0u;scale<3u;scale+=1u){
        ...
        if scale==2u {
            ...
            let a=support_info(dirs[1],i);let b=support_info(dirs[3],i);
