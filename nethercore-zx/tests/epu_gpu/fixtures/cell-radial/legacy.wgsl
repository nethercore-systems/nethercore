...
fn cell_radial(uv: vec2f, density: f32, axis: vec3f, dir: vec3f) -> vec3f {
    ...
    let radial_uv = radial_legacy_epu_wrapped_relief_uv(
    ...
    let ring_relief = radial_legacy_epu_relief_envelope(radius_phase, 0.06, 0.22, 0.82, 0.99);
    let angular_relief = radial_legacy_epu_relief_wave(vec2f(t_proj, b_proj) * vec2f(2.4, 2.1), density * 0.017);
    let radial_relief = radial_legacy_epu_relief_wave(
    ...
    let edge_relief = radial_legacy_epu_relief_wave(vec2f(radius_phase * ring_count, angle_phase * spoke_count), density * 0.061 + 0.23);
    let ring_edge = mix(
        0.5,
        radial_legacy_epu_periodic_edge_distance(ring_phase + edge_relief * 0.08 * ring_relief),
        ring_relief
    );
    let spoke_edge = mix(
        0.5,
        radial_legacy_epu_periodic_edge_distance(spoke_phase + edge_relief * 0.06 * center_relief),
        center_relief
    );
...
fn eval_cell(
    ...
    switch variant {
        case 3u: { // RADIAL
            let radial_center = radial_legacy_epu_relief_envelope(uv.y, 0.08, 0.26, 0.78, 0.98);
            let radial_gate = smoothstep(
                -0.25,
                0.7,
                radial_legacy_epu_relief_wave(vec2f(cell_id.x * 0.37, uv.y * density * 0.91 + cell_id.y * 0.11), seed * 0.017)
            );
    ...
    return BoundsResult(LayerSample(rgb, radial_legacy_epu_saturate(w)), output_regions, 1.0);
}

fn radial_legacy_epu_periodic_centered(x: f32) -> f32 {
    return fract(x + 0.5) - 0.5;
}

fn radial_legacy_epu_periodic_edge_distance(x: f32) -> f32 {
    return 0.5 - abs(radial_legacy_epu_periodic_centered(x));
}

fn radial_legacy_epu_relief_envelope(x: f32, lo0: f32, lo1: f32, hi0: f32, hi1: f32) -> f32 {
    return smoothstep(lo0, lo1, x) * (1.0 - smoothstep(hi0, hi1, x));
}

fn radial_legacy_epu_relief_wave(p: vec2f, phase: f32) -> f32 {
...
fn radial_legacy_epu_saturate(x: f32) -> f32 { return clamp(x, 0.0, 1.0); }

fn radial_legacy_epu_wrapped_relief_uv(uv: vec2f, phase: f32, u_amount: f32, v_amount: f32) -> vec2f {
    let uc = radial_legacy_epu_periodic_centered(uv.x);
    let seam_gate = smoothstep(0.025, 0.12, radial_legacy_epu_periodic_edge_distance(uv.x));
    ...
    let wave0 = radial_legacy_epu_relief_wave(p0, phase);
    let wave1 = radial_legacy_epu_relief_wave(p1, phase + 0.37);
