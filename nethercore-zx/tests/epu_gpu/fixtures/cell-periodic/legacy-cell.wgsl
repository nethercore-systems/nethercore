...
//   param_a: Cell density (0..255 -> 4..64); HEX rounds to whole cell columns
...
fn cell_brick(uv: vec2f, density: f32, seed: f32) -> vec3f {
    ...
    let cell_id = vec2f(cell_offset_id_x(floor(offset_x), density), row);
...
fn eval_cell(
    ...
    let is_solid = select(cell_value < fill_ratio, cell_info.z > -99.0, variant == 1u);
