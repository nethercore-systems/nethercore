...
const metric_CELESTIAL_VARIANT_MOON: u32 = 0u;       // Cratered surface, phase illumination
const metric_CELESTIAL_VARIANT_SUN: u32 = 1u;        // Corona/glow, limb darkening
const metric_CELESTIAL_VARIANT_PLANET: u32 = 2u;     // Solid body with optional cloud bands
const metric_CELESTIAL_VARIANT_GAS_GIANT: u32 = 3u;  // Jupiter-like horizontal stripes
const metric_CELESTIAL_VARIANT_RINGED: u32 = 4u;     // Saturn-like with tilted rings
const metric_CELESTIAL_VARIANT_BINARY: u32 = 5u;     // Two bodies (primary + secondary)
const metric_CELESTIAL_VARIANT_ECLIPSE: u32 = 6u;    // Solar eclipse with corona

// Deterministic hash for celestial noise (2D -> 1D)
fn metric_celestial_hash21(p: vec2f) -> f32 {
...
fn metric_celestial_noise(p: vec2f) -> f32 {
    let i = floor(p);
    let f = fract(p);
    let u = f * f * (3.0 - 2.0 * f); // smoothstep

    let a = metric_celestial_hash21(i);
    let b = metric_celestial_hash21(i + vec2f(1.0, 0.0));
    let c = metric_celestial_hash21(i + vec2f(0.0, 1.0));
    let d = metric_celestial_hash21(i + vec2f(1.0, 1.0));

    return mix(mix(a, b, u.x), mix(c, d, u.x), u.y);
}

// FBM noise for surface detail (2 octaves for mip stability)
fn metric_celestial_fbm(p: vec2f) -> f32 {
    var value = 0.0;
    var amplitude = 0.5;
    var pp = p;

    for (var i = 0u; i < 2u; i++) {
        value += amplitude * metric_celestial_noise(pp);
        amplitude *= 0.5;
...
fn metric_celestial_surface_uv(dir: vec3f, body_dir: vec3f, r: f32) -> vec2f {
...
fn metric_eval_celestial_moon(
    ...
    let crater_noise = metric_celestial_fbm(surface_uv * 8.0);
...
fn metric_eval_celestial_planet(
    r: f32,
    surface_uv: vec2f,
    phase_factor: f32,
    limb: f32,
    ...
    let terrain = metric_celestial_fbm(surface_uv * 4.0);
...
fn metric_eval_celestial_gas_giant(
    r: f32,
    surface_uv: vec2f,
    limb: f32,
    color_a: vec3f,
    ...
    let turbulence = metric_celestial_noise(surface_uv * 12.0 + vec2f(latitude * 4.0, 0.0)) * 0.15;
...
fn metric_eval_celestial_ringed(
    dir: vec3f,
    body_dir: vec3f,
    r: f32,
...
fn metric_eval_celestial_binary(
    dir: vec3f,
    body_dir: vec3f,
    angular_size_rad: f32,
    limb_exp: f32,
...
fn metric_eval_celestial_eclipse(
    r: f32,
    color_a: vec3f,
    color_b: vec3f,
...
fn metric_eval_celestial(
    ...
    let surface_uv = metric_celestial_surface_uv(dir, body_dir, sin(angular_size_rad));
    ...
    switch variant_id {
        case metric_CELESTIAL_VARIANT_MOON: {
            sample = metric_eval_celestial_moon(r, surface_uv, phase_factor, limb, color_a, color_b, alpha_b);
        }
        case metric_CELESTIAL_VARIANT_SUN: {
            // param_d: Corona extent (0..255 -> 1.0..3.0)
            let corona_extent = mix(1.0, 3.0, param_d_raw);
            sample = metric_eval_celestial_sun(r, limb, color_a, color_b, corona_extent, alpha_b);
        }
        case metric_CELESTIAL_VARIANT_PLANET: {
            // param_d: Cloud band count (0..255 -> 0..8)
            let band_count = param_d_raw * 8.0;
            sample = metric_eval_celestial_planet(r, surface_uv, phase_factor, limb, color_a, color_b, band_count, alpha_b);
        }
        case metric_CELESTIAL_VARIANT_GAS_GIANT: {
            // param_d: Horizontal band count (0..255 -> 2..16)
            let band_count = mix(2.0, 16.0, param_d_raw);
            sample = metric_eval_celestial_gas_giant(r, surface_uv, limb, color_a, color_b, band_count, alpha_b);
        }
        case metric_CELESTIAL_VARIANT_RINGED: {
            // param_d: Ring tilt (0..255 -> 0..90 degrees)
            let ring_tilt = param_d_raw * 90.0;
            sample = metric_eval_celestial_ringed(dir, body_dir, r, angular_size_rad, limb, color_a, color_b, ring_tilt, alpha_b);
        }
        case metric_CELESTIAL_VARIANT_BINARY: {
            // param_d: Secondary size ratio (0..255 -> 0.2..2.0)
            let size_ratio = mix(0.2, 2.0, param_d_raw);
            sample = metric_eval_celestial_binary(dir, body_dir, angular_size_rad, limb_exp, color_a, color_b, size_ratio, alpha_b);
        }
        case metric_CELESTIAL_VARIANT_ECLIPSE: {
            // param_d: Corona brightness (0..255 -> 1.0..2.5)
            let corona_brightness = mix(1.0, 2.5, param_d_raw);
            sample = metric_eval_celestial_eclipse(r, color_a, color_b, corona_brightness, alpha_b);
        }
        default: {
