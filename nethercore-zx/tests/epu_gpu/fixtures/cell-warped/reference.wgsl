// Frozen qualified prototype; do not regenerate from production.
fn warpref_organic_fused_epu_relief_wave(p: vec2f, phase: f32) -> f32 {
...
fn warpref_organic_fused_epu_wrapped_relief_uv(uv: vec2f, phase: f32, u_amount: f32, v_amount: f32) -> vec2f {
    ...
    let wave0 = warpref_organic_fused_epu_relief_wave(p0, phase);
    let wave1 = warpref_organic_fused_epu_relief_wave(p1, phase + 0.37);
...
fn warpref_cell_hash_bits(cell: vec2f, seed: f32, salt: u32) -> u32 {
...
fn warpref_cell_hash2(cell: vec2f, seed: f32) -> f32 {
    return f32(warpref_cell_hash_bits(cell, seed, 0u) >> 8u) / 16777216.0;
}

fn warpref_cell_hash2_vec2(cell: vec2f, seed: f32) -> vec2f {
    return vec2f(
        f32(warpref_cell_hash_bits(cell, seed, 0u) >> 8u),
        f32(warpref_cell_hash_bits(cell, seed, 0x9e3779b9u) >> 8u)
    ) / 16777216.0;
}

fn warpref_wrap_periodic_x(x: f32, period: f32) -> f32 {
    return fract(x / period) * period;
}

fn warpref_shortest_periodic_delta(x: f32, center: f32, period: f32) -> f32 {
    return x - center - round((x - center) / period) * period;
}

// Compute axis-cylinder UV from direction and alignment axis
fn warpref_cell_axis_cylinder_uv(dir: vec3f, axis: vec3f) -> vec2f {
...
fn warpref_cell_density(byte: u32) -> f32 {
    let steps = byte * 4u;
...
fn warpref_cell_grid(uv: vec2f, density: f32) -> vec3f {
...
fn warpref_cell_offset_id_x(x: f32, density: f32) -> f32 {
...
fn warpref_cell_hex_fields(uv: vec2f, density: f32, seed: f32, fill: f32) -> vec4f {
    ...
    for (var dy = -1; dy <= 1; dy += 1) {
        ...
        for (var dx = -1; dx <= 1; dx += 1) {
            ...
            let delta = p - center;
            let id = vec2f(warpref_cell_offset_id_x(f32(col), count), f32(row));
            let distance = dot(delta, delta);
            if distance < nearest { nearest = distance; owner = id; }
            let q = abs(delta);
            let edge = 0.5 - max(q.x, 0.5 * q.x + row_height * q.y);
            all_edge = max(all_edge, edge);
            if warpref_cell_hash2(id, seed) < fill { solid_edge = max(solid_edge, edge); }
        }
    }
    return vec4f(owner, all_edge, solid_edge);
}
fn warpref_cell_hex(uv: vec2f, density: f32) -> vec3f {
    return warpref_cell_hex_fields(uv, density, 0.0, 1.0).xyz;
}

// BRICK: ordinary staggered rectangles; whole periodic columns, no hidden relief.
fn warpref_cell_brick(uv: vec2f, density: f32, seed: f32) -> vec3f {
    let count = round(density);
    ...
    let id = vec2f(warpref_cell_offset_id_x(floor(p.x), count), row);
    let local = fract(p);
...
fn warpref_cell_rect_fields(uv: vec2f, density: f32, seed: f32, fill: f32, brick: bool) -> vec4f {
    let count = select(density, round(density), brick);
    let height = select(count, count * 0.5, brick);
    var info = warpref_cell_grid(uv, density);
    if brick { info = warpref_cell_brick(uv, density, seed); }
...
fn warpref_cell_grid_fields(uv: vec2f, density: f32, seed: f32, fill: f32) -> vec4f {
    return warpref_cell_rect_fields(uv, density, seed, fill, false);
}

fn warpref_cell_brick_fields(uv: vec2f, density: f32, seed: f32, fill: f32) -> vec4f {
    return warpref_cell_rect_fields(uv, density, seed, fill, true);
}
...
fn warpref_cell_site_fields(uv: vec2f, period: f32, seed: f32, jitter_scale: f32, fill_seed: f32, fill: f32) -> vec4f {
    ...
    for (var dy = -2; dy <= 2; dy += 1) {
        for (var dx = -2; dx <= 2; dx += 1) {
            ...
            let jitter = warpref_cell_hash2_vec2(id, seed);
            let point = id + jitter * jitter_scale + vec2f(select(0.1, 0.0, jitter_scale == 1.0));
            let d = length(vec2f(warpref_shortest_periodic_delta(scaled.x, point.x, period), scaled.y - point.y));
            if warpref_cell_hash2(id, fill_seed) < fill { nearest_filled = min(nearest_filled, d); }
            if d < first {
                second = first;
                first = d;
                owner = id;
    ...
    var solid_edge = -100.0;
    if warpref_cell_hash2(owner, fill_seed) < fill {
        solid_edge = edge;
...
fn warpref_cell_periodic_candidates(uv: vec2f, period: f32, seed: f32, jitter_scale: f32) -> vec3f {
    return warpref_cell_site_fields(uv, period, seed, jitter_scale, seed, 0.0).xyz;
}

fn warpref_cell_voronoi(uv: vec2f, density: f32, seed: f32) -> vec3f {
    return warpref_cell_periodic_candidates(uv, round(density), seed, 0.8);
}

// SHATTER variant: voronoi with higher jitter
fn warpref_cell_shatter(uv: vec2f, density: f32, seed: f32) -> vec3f {
    return warpref_cell_periodic_candidates(uv, round(density), seed + 42.0, 1.0);
}
...
fn warpref_organic_smooth_hash2(p: vec2f, seed: f32) -> f32 {
    let q=p-vec2f(.5);let i=floor(q);let f=fract(q);let w=f*f*(vec2f(3.)-2.*f);
    return mix(mix(warpref_cell_hash2(i,seed),warpref_cell_hash2(i+vec2f(1.,0.),seed),w.x),
        mix(warpref_cell_hash2(i+vec2f(0.,1.),seed),warpref_cell_hash2(i+vec2f(1.,1.),seed),w.x),w.y);
}
// C1-periodic extension of the retained relief and jitter fields.
fn warpref_organic_periodic_wave(p:vec2f, period:vec2f, u:f32, seed:f32)->f32 {
 return mix(warpref_organic_fused_epu_relief_wave(p,seed),warpref_organic_fused_epu_relief_wave(p-period,seed),smoothstep(0.,1.,u));
}
fn warpref_organic_periodic_hash(p:vec2f, period:vec2f, u:f32, seed:f32)->f32 {
 return mix(warpref_organic_smooth_hash2(p,seed),warpref_organic_smooth_hash2(p-period,seed),smoothstep(0.,1.,u));
}
fn warpref_organic_radial_phases(uv: vec2f, density: f32, axis: vec3f, dir: vec3f) -> vec2f {
    ...
    let radial_uv = warpref_organic_fused_epu_wrapped_relief_uv(
    ...
    let angular_relief = warpref_organic_fused_epu_relief_wave(vec2f(t_proj, b_proj) * vec2f(2.4, 2.1), density * 0.017);
    ...
    let radial_relief = warpref_organic_periodic_wave(
        vec2f(angle_phase,radius_phase)*relief_counts,vec2f(relief_counts.x,0.),angle_phase,
        density * 0.043 + z_proj * 0.5
    );
    let ring_warp = (warpref_organic_periodic_hash(vec2f(angle_phase * spoke_count, radius_phase * ring_count),vec2f(spoke_count,0.),angle_phase,density * 0.37) - 0.5)
        * mix(0.0, 0.16 / ring_count, ring_relief);
    let spoke_warp = (warpref_organic_periodic_hash(vec2f(radius_phase * ring_count, angle_phase * spoke_count),vec2f(0.,spoke_count),angle_phase,density * 0.73) - 0.5)
        * (0.14 / spoke_count)
        * center_relief;
    ...
    let edge_relief = warpref_organic_periodic_wave(vec2f(radius_phase * relief_counts.y, angle_phase * relief_counts.x),vec2f(0.,relief_counts.x),angle_phase,density * 0.061 + 0.23);
    let ring_edge = mix(
        0.5,
...
fn warpref_cell_radial_fields(uv:vec2f, density:f32, axis:vec3f, dir:vec3f, seed:f32, fill:f32)->vec4f {
 let phase=warpref_organic_radial_phases(uv,density,axis,dir);
 let x=phase.x;let y=phase.y;
 let col=i32(floor(x+0.5));let row=max(0,i32(floor(y+0.5)));
 var owner=vec2f(warpref_cell_offset_id_x(f32(col),round(density)),f32(row));if row==0 {owner=vec2f(0.);}
 let center_edge=0.5-y;
 var all_edge=center_edge;
 var solid_edge=select(-100.,center_edge,warpref_cell_hash2(vec2f(0.),seed)<fill);
 for(var dy=-1;dy<=1;dy+=1){
  let r=row+dy;if r<1 {continue;}
  for(var dx=-1;dx<=1;dx+=1){
   let c=col+dx;let id=vec2f(warpref_cell_offset_id_x(f32(c),round(density)),f32(r));
...
fn warpref_cell_radial(uv: vec2f, density: f32, axis: vec3f, dir: vec3f) -> vec3f {
    return warpref_cell_radial_fields(uv, density, axis, dir, 0.0, 1.0).xyz;
}

fn warpref_eval_cell(
    dir: vec3f,
    instr: vec4u,
    ...
    let density = warpref_cell_density(instr_a(instr));
    ...
    let uv = warpref_cell_axis_cylinder_uv(dir, axis);

    // Compute cell info based on variant: vec3(cell_id.xy, d_edge)
    var cell_info: vec3f;
    switch variant {
        case 0u: { let fields = warpref_cell_grid_fields(uv, density, seed, fill_ratio); cell_info = vec3f(fields.xy, fields.w); } // GRID
        case 1u: { let fields = warpref_cell_hex_fields(uv, density, seed, fill_ratio); cell_info = vec3f(fields.xy, fields.w); }       // HEX
        case 2u: { let fields = warpref_cell_site_fields(uv, round(density), seed, 0.8, seed, fill_ratio); cell_info = vec3f(fields.xy, fields.w); }  // VORONOI
        case 3u: { let fields = warpref_cell_radial_fields(uv, density, axis, dir, seed, fill_ratio); cell_info = vec3f(fields.xy, fields.w); } // RADIAL
        case 4u: { let fields = warpref_cell_site_fields(uv, round(density), seed + 42.0, 1.0, seed, fill_ratio); cell_info = vec3f(fields.xy, fields.w); }  // SHATTER
        case 5u: { let fields = warpref_cell_brick_fields(uv, density, seed, fill_ratio); cell_info = vec3f(fields.xy, fields.w); } // BRICK
        default: { let fields = warpref_cell_grid_fields(uv, density, seed, fill_ratio); cell_info = vec3f(fields.xy, fields.w); }
    }
    ...
    let cell_value = warpref_cell_hash2(cell_id, seed);
