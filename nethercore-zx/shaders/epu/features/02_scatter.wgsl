...
fn scatter_twinkle_mod(variant: u32, base_twinkle: f32, h: f32, time: f32, twinkle_speed: f32) -> f32 {
    switch variant {
        ...
        case SCATTER_VARIANT_DUST: {
            // Slow, subtle twinkle
            let tw = 0.8 + 0.2 * sin(h * TAU + time * twinkle_speed * 0.3);
            return mix(1.0, tw, base_twinkle * 0.5);
        }
        case SCATTER_VARIANT_BUBBLES: {
            // Gentle shimmer
            let tw = 0.7 + 0.3 * sin(h * TAU + time * twinkle_speed * 0.5);
            return mix(1.0, tw, base_twinkle * 0.7);
        }
        case SCATTER_VARIANT_EMBERS: {
            // Flickering fade
            let tw = 0.3 + 0.7 * sin(h * TAU + time * twinkle_speed * 2.0);
            return tw * tw; // Sharper flicker
        }
        case SCATTER_VARIANT_RAIN: {
            // Slight variation
            return 0.9 + 0.1 * sin(h * TAU + time * twinkle_speed);
        }
        case SCATTER_VARIANT_SNOW: {
            // Very subtle shimmer
            let tw = 0.9 + 0.1 * sin(h * TAU + time * twinkle_speed * 0.2);
            return mix(1.0, tw, base_twinkle * 0.3);
        }
        default: {
            // STARS: standard twinkle
            let tw = 0.5 + 0.5 * sin(h * TAU + time * twinkle_speed);
            return select(1.0, tw, base_twinkle > 0.001);
        }
    }
}
...
fn eval_scatter(
    ...
    let tw = scatter_twinkle_mod(variant_id, twinkle, h.w, time, twinkle_speed);
