//! Asset loading FFI for NetherZ binary formats (.nczxmesh, .nczxtex, .nczxsnd, .nczxskel)
//!
//! These functions load assets from POD NetherZ binary formats.
//! No magic bytes - format is determined by which function is called.
//!
//! Usage from game code:
//! ```text
//! static MESH_DATA: &[u8] = include_bytes!("player.nczxmesh");
...
use super::{ZXGameContext, get_wasm_memory, guards::guard_init_only};
...
pub fn register(linker: &mut Linker<ZXGameContext>) -> Result<()> {
    // Load from NetherZ binary formats (Phase A - include_bytes! workflow)
...
/// Load a mesh from NetherZ mesh (.nczxmesh) binary format
///
/// # Arguments
...
fn load_zmesh(mut caller: Caller<'_, ZXGameContext>, data_ptr: u32, data_len: u32) -> u32 {
    guard_init_only!(caller, "load_zmesh");
...
/// Load a texture from NetherZ texture (.nczxtex) binary format
///
/// # Arguments
/// * `data_ptr` — Pointer to .nczxtex binary data
/// * `data_len` — Length of the data in bytes
///
...
fn load_ztex(mut caller: Caller<'_, ZXGameContext>, data_ptr: u32, data_len: u32) -> u32 {
    guard_init_only!(caller, "load_ztex");
    ...
    let (width, height, pixel_data) = {
        ...
        let Some(header) = NetherZXTextureHeader::from_bytes(data) else {
            warn!(
                ...
                Regenerate with: nether-export texture <source.png> -o <output.nczxtex>"
            );
            return 0;
        };
...
fn load_zsound(mut caller: Caller<'_, ZXGameContext>, data_ptr: u32, data_len: u32) -> u32 {
    guard_init_only!(caller, "load_zsound");
...
/// Load a skeleton from NetherZ skeleton (.nczxskel) binary format
///
/// # Arguments
/// * `data_ptr` — Pointer to .nczxskel binary data
/// * `data_len` — Length of the data in bytes
///
...
fn load_zskeleton(mut caller: Caller<'_, ZXGameContext>, data_ptr: u32, data_len: u32) -> u32 {
    guard_init_only!(caller, "load_zskeleton");
