...
fn xm_envelopes_transition_stereo_gains_within_the_tick() {
    ...
    for (ft2, volume, pan, curve, expected) in [
        ...
        for lane in 0..2 {
            assert!(
                (mean[lane] - expected[lane]).abs() <= (expected[lane] * 0.02).max(2.0),
                "ft2={ft2} volume={volume} pan={pan} lane={lane}: {} vs {}",
                mean[lane],
                expected[lane]
            );
        }
        if !curve {
            ...
            assert_eq!(
                changed_rate, settled,
                "a longer tick cannot revive a completed gain transition"
            );
            ...
            for _ in 0..2920 {
                changed.render_sample_and_advance(&mut pos, &sounds, 96000);
            }
            ...
            assert_eq!(
                shorter, longer,
                "shortening then lengthening cannot revive an exhausted ramp"
            );
            // Instrument-only selection resets envelopes without restarting the sample.
            let saved_gain = engine.channels[0]
                .xm_envelope_ramp
                .as_ref()
                .unwrap()
                .current;
            ...
            engine.mix_channels(
                crate::tracker::raw_tracker_handle(handle),
                &sounds,
                44100,
                882,
            );
            assert_eq!(
                engine.channels[0]
                    ...
                    .current,
                saved_gain,
                "envelope-only reset must preserve stopped residual gain"
            );
        }
    }
}
...
fn xm_pan_and_preamp_follow_supplied_module_and_survive_snapshot() {
    ...
    let linear = engine.mix_channels(
        crate::tracker::raw_tracker_handle(compatible),
        &sounds,
        44100,
        882,
    );
    let legacy_pcm = engine.mix_channels(
        crate::tracker::raw_tracker_handle(legacy),
        &sounds,
        44100,
        882,
    );
    for value in [legacy_pcm.0, legacy_pcm.1] {
        assert!((value - 0.25 * (97.0 / 128.0) * 0.5 * (2.0 / 3.0)).abs() < 0.000001);
    }
    let expected_equal = 0.25 * (48.0 / 128.0) * 0.5f32.sqrt();
    let expected_linear = 0.25 * (96.0 / 128.0) * 0.5;
    for sample in [equal.0, equal.1] {
        assert!((sample - expected_equal).abs() < 0.000001, "{equal:?}");
    }
    for sample in [linear.0, linear.1] {
        assert!((sample - expected_linear).abs() < 0.000001, "{linear:?}");
    }
    let mut receiver = TrackerEngine::new();
    receiver.apply_snapshot(&engine.snapshot());
    assert_eq!(
        receiver.mix_channels(crate::tracker::raw_tracker_handle(ft2), &sounds, 44100, 882),
        equal
    );
    assert_eq!(
        receiver.mix_channels(
            crate::tracker::raw_tracker_handle(compatible),
            &sounds,
            44100,
            882
...
fn it_balance_pan_uses_supplied_handle_and_survives_snapshot() {
    ...
    song.format = song.format
        | FormatFlags::IT_BALANCE_MIX
        | FormatFlags::XM_LEGACY_MIX
        | FormatFlags::XM_LEGACY_RETRIGGER;
    ...
    for pan in [-1.0, 0.0, 1.0] {
        ...
        let a = engine.mix_channels(
            crate::tracker::raw_tracker_handle(ordinary),
            &sounds,
            44100,
            882,
        );
        let b = engine.mix_channels(
            crate::tracker::raw_tracker_handle(balance),
            &sounds,
            44100,
            882,
        );
