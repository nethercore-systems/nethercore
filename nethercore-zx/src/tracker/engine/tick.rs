...
use super::super::utils::{apply_it_linear_slide, get_waveform_value, note_to_period, slide_xm_period, xm_sample_finetune, xm_vibrato_period};
...
fn advance_envelope(pos: u16, sustain: Option<(u16, u16)>, normal: Option<(u16, u16)>, released: bool, it: bool) -> u16 {
    if it {
        let next = u32::from(pos) + 1;
        let active = if !released { sustain.or(normal) } else { normal };
        if let Some((start, end)) = active {
            if next > u32::from(end) { return start; }
        }
...
mod envelope_tests {
    ...
    fn inclusive_it_endpoints_and_legacy_xm_ordering() {
        ...
        assert_eq!(advance_envelope(3, Some((3, 3)), Some((0, 3)), false, true), 3);
        assert_eq!(advance_envelope(3, Some((3, 3)), Some((0, 3)), true, true), 0);
        assert_eq!(advance_envelope(u16::MAX, None, Some((1, u16::MAX)), false, true), 1);
        assert_eq!(advance_envelope(u16::MAX, None, None, false, true), u16::MAX);
    }
}
...
impl TrackerEngine {
    ...
    fn advance_envelopes_except(&mut self, new_voices: &[bool; MAX_TRACKER_CHANNELS]) {
        for (index, channel) in self.channels.iter_mut().enumerate() {
            ...
            if channel.volume_envelope_enabled {
                ...
                channel.volume_envelope_pos = advance_envelope(channel.volume_envelope_pos,
                    channel.volume_envelope_sustain_loop,
                    if !self.is_it_format && channel.key_off && channel.volume_envelope_escape_loop {
                        None
                    } else { channel.volume_envelope_loop },
                    channel.key_off, self.is_it_format);
            }
            ...
            if channel.panning_envelope_enabled && !channel.panning_envelope_frozen {
                ...
                channel.panning_envelope_pos = advance_envelope(channel.panning_envelope_pos,
                    channel.panning_envelope_sustain_loop,
                    if !self.is_it_format && channel.key_off && channel.panning_envelope_escape_loop {
                        None
                    } else { channel.panning_envelope_loop },
                    channel.key_off, self.is_it_format);
                channel.panning_envelope_frozen = !self.is_it_format && !channel.key_off
                    && channel.panning_envelope_sustain_loop.is_some_and(|(start, end)|
            ...
            if channel.pitch_envelope_enabled {
                ...
                channel.pitch_envelope_pos = advance_envelope(channel.pitch_envelope_pos,
                    channel.pitch_envelope_sustain_loop, channel.pitch_envelope_loop,
                    channel.key_off, self.is_it_format);
            }
            ...
            if channel.filter_envelope_enabled {
                ...
                channel.filter_envelope_pos = advance_envelope(channel.filter_envelope_pos,
                    channel.filter_envelope_sustain_loop, channel.filter_envelope_loop,
                    channel.key_off, self.is_it_format);
            }
            ...
            if self.is_it_format && channel.volume_envelope_enabled
                && channel.volume_envelope_loop.is_none()
                && channel.volume_envelope_end.is_some_and(|end| channel.volume_envelope_pos > end)
            {
                channel.note_fade = true;
            }
            let release_fades = channel.key_off && (!self.is_it_format
                || !channel.volume_envelope_enabled || channel.volume_envelope_loop.is_some());
    ...
    pub(super) fn advance_xm_auto_vibrato(&mut self) {
        if self.is_it_format { return; }
        for channel in &mut self.channels {
            if !channel.note_on || channel.auto_vibrato_depth == 0 { continue; }
            channel.auto_vibrato_pos = (channel.auto_vibrato_pos + u16::from(channel.auto_vibrato_rate)) & 255;
            if !channel.key_off {
            ...
            let wave = match channel.auto_vibrato_type {
                ...
                1 => if phase < 128 { -1.0 } else { 1.0 },
                2 => f32::from(phase as i8) / 128.0,
                3 => f32::from(phase.wrapping_sub(1).wrapping_neg() as i8) / 128.0,
                _ => 0.0,
            };
            let sweep = if channel.key_off && channel.auto_vibrato_sweep < u16::from(channel.auto_vibrato_sweep_len) { 0.0 }
            else if channel.auto_vibrato_sweep_len == 0 { 1.0 } else {
                (f32::from(channel.auto_vibrato_sweep) / f32::from(channel.auto_vibrato_sweep_len)).min(1.0)
            };
            let delta = (wave * f32::from(channel.auto_vibrato_depth) * sweep).trunc();
            channel.period = slide_xm_period(channel.period, delta, channel.xm_amiga_slides, channel.xm_source_tuning);
        }
    }
    ...
    pub fn process_tick(&mut self, tick: u16, speed: u16) {
        ...
        for ch_idx in 0..MAX_TRACKER_CHANNELS {
            let skip_delayed_volume_slide = !self.is_it_format
                ...
                && self.channels[ch_idx].delayed_xm_note
                    .is_some_and(|(note, _)| note.instrument != 0);
            if self.channels[ch_idx].note_delay_tick as u16 == tick {
                if let Some((mut note, handle)) = self.channels[ch_idx].delayed_xm_note.take() {
                    ...
                    if tick < delay_limit {
                        if !self.is_it_format && note.note == 0
                        ...
                        if !self.is_it_format
                            && matches!(note.volume_effect, nether_tracker::TrackerEffect::TonePortamento(_))
                        {
                        ...
                        new_voices[ch_idx] = self.process_note_internal(ch_idx, &note, handle, &[]);
                    }
                }
            }
            ...
            if channel.arpeggio_active || channel.vibrato_active || (!self.is_it_format && channel.auto_vibrato_depth > 0) {
                channel.period = channel.base_period;
            }
            // IT retains its existing tick ordering.
            if self.is_it_format && channel.arpeggio_active
                && (channel.arpeggio_note1 != 0 || channel.arpeggio_note2 != 0)
            {
                ...
                } else { (speed.saturating_sub(tick) % 3) as u8 };
            ...
            if !self.is_it_format && !skip_delayed_volume_slide {
                channel.volume = (channel.volume + f32::from(channel.xm_volume_column_slide) / 64.0).clamp(0.0, 1.0);
            }
            // Volume slide
            ...
            if channel.tone_porta_active && channel.target_period > 0.0 && channel.porta_speed > 0 {
                ...
                if self.is_it_format {
                    ...
                    let next = slide_xm_period(channel.period, channel.porta_speed as f32 * 4.0, channel.xm_amiga_slides, channel.xm_source_tuning);
                    channel.period = next.min(channel.target_period);
                } else if diff != 0.0 {
                    let speed = channel.porta_speed as f32 * 4.0;
                    let next = slide_xm_period(channel.period, speed.copysign(diff), channel.xm_amiga_slides, channel.xm_source_tuning);
                    channel.period = if diff > 0.0 { next.min(channel.target_period) }
                        else { next.max(channel.target_period) };
                }
                if !self.is_it_format && !channel.xm_legacy_retrigger && diff < 0.0 && channel.period == channel.target_period {
            ...
            if !self.is_it_format {
                ...
                if channel.arpeggio_active && (channel.arpeggio_note1 != 0 || channel.arpeggio_note2 != 0) {
                    ...
                    let offset = match channel.arpeggio_tick { 0=>0, 1=>channel.arpeggio_note1, _=>channel.arpeggio_note2 };
            ...
            if self.is_it_format { channel.advance_it_modulation(false, self.old_effects_mode); }
            ...
            if !self.is_it_format && channel.xm_multi_retrigger_active {
            ...
            } else if !self.is_it_format && channel.retrigger_tick > 0 && tick.is_multiple_of(channel.retrigger_tick as u16) {
                ...
                if !self.is_it_format {
                    ...
                    let retrigger_note = if channel.xm_retrigger_note == 0 {channel.current_note} else {channel.xm_retrigger_note};
                    channel.period = note_to_period(retrigger_note, channel.finetune)
                        - f32::from(delta) / 2.0;
                    channel.base_period = channel.period;
                    // Measured legacy E9 refreshes the active target; FT2 retains it.
                    if channel.xm_legacy_retrigger && channel.tone_porta_active && channel.target_period > 0.0 {
                    ...
                    new_voices[ch_idx] = true;
                }
            }

            if channel.panning_left_on_ticks && !skip_delayed_volume_slide { channel.panning = -1.0; }

            // XM columns execute independently before main-column Pxy.
            if !self.is_it_format && channel.xm_volume_column_pan_slide != 0
                && (channel.xm_legacy_retrigger || !skip_delayed_volume_slide) {
                let scale = if channel.xm_legacy_retrigger {32.0} else {128.0};
                let upper = if channel.xm_legacy_retrigger {1.0} else {127.0/128.0};
                channel.panning = (channel.panning + channel.xm_volume_column_pan_slide as f32 / scale).clamp(-1.0,upper);
            }
            if channel.panning_slide_active && channel.panning_slide != 0 {
                let (scale,upper) = if self.is_it_format { (255.0,1.0) }
                    else if channel.xm_legacy_retrigger { (32.0,1.0) } else { (128.0,127.0/128.0) };
                channel.panning = (channel.panning + channel.panning_slide as f32 / scale).clamp(-1.0,upper);
            }
            ...
            if channel.tremor_active && !self.is_it_format {
                ...
                let duration = if channel.tremor_mute { channel.tremor_off_ticks }
                    else { channel.tremor_on_ticks };
            ...
            if channel.note_cut_tick > 0 && tick == channel.note_cut_tick as u16 {
                channel.volume = 0.0;
                if self.is_it_format { channel.note_on = false; }
            }
            ...
            if channel.key_off_tick > 0 && tick == channel.key_off_tick as u16 {
                ...
                if !self.is_it_format && !channel.volume_envelope_enabled { channel.volume = 0.0; }
            }
