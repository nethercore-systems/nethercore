...
fn test_matrix_packing_multiple_frames() {
    ...
    println!("Frame 1 - Added matrices at indices: {}, {}", idx1_a, idx1_b);
    ...
    z_state.clear_frame();
    println!("Frame 1 - After clear: model_matrices.len() = {}", z_state.model_matrices.len());
    assert_eq!(z_state.model_matrices.len(), 0, "Model matrices should be cleared");
    assert_eq!(z_state.current_model_idx, 0, "current_model_idx should be reset");
    ...
    println!("Frame 2 - Added matrices at indices: {}, {}", idx2_a, idx2_b);
    assert_eq!(idx2_a, 0, "Frame 2: First matrix should be at index 0 (reused)");
    assert_eq!(idx2_b, 1, "Frame 2: Second matrix should be at index 1 (reused)");
    assert_eq!(z_state.model_matrices.len(), 2);

    // Verify the matrices are the NEW ones, not the old ones
    assert_eq!(z_state.model_matrices[0], model2_a, "Should have new matrix, not old");
    assert_eq!(z_state.model_matrices[1], model2_b, "Should have new matrix, not old");
    assert_ne!(z_state.model_matrices[0], model1_a, "Should NOT have old matrix");
}
...
fn test_deferred_command_matrix_indices() {
    ...
    let billboard_mvp = MvpIndex::new(billboard_idx, z_state.current_view_idx, z_state.current_proj_idx);

    println!("Billboard added at index: {}", billboard_idx);
    assert_eq!(billboard_idx, 2, "Billboard should be at index 2 (after A and B)");
...
fn test_render_pass_swap_preserves_indices() {
    ...
    println!("Before swap:");
    println!("  z_state.render_pass commands: {}", z_state.render_pass.commands().len());
    println!("  graphics_command_buffer commands: {}", graphics_command_buffer.commands().len());
...
fn test_full_pipeline_multiple_frames() {
    ...
    println!("Frame 1: Added model matrices at indices {} and {}", idx1_a, idx1_b);
    ...
    std::mem::swap(&mut graphics_command_buffer, &mut z_state.render_pass);
    println!("Frame 1: Swapped render pass - graphics_command_buffer now has {} commands", graphics_command_buffer.commands().len());
    ...
    println!("Frame 1: After deferred expansion - {} commands, {} model matrices",
    ...
    assert_eq!(cmd0_mvp.0, 0, "First command should reference model matrix 0");
    assert_eq!(cmd1_mvp.0, 1, "Second command should reference model matrix 1");
    ...
    z_state.clear_frame();
    println!("Frame 1: Cleared - model_matrices.len() = {}", z_state.model_matrices.len());
    ...
    println!("Frame 2: Added model matrices at indices {} and {} (should be 0 and 1)", idx2_a, idx2_b);
    ...
    std::mem::swap(&mut graphics_command_buffer, &mut z_state.render_pass);
    println!("Frame 2: After swap - graphics_command_buffer has {} commands", graphics_command_buffer.commands().len());
    println!("Frame 2: z_state.render_pass (old graphics_command_buffer) has {} commands", z_state.render_pass.commands().len());

    // The old commands from Frame 1 should now be in z_state.render_pass
    assert_eq!(z_state.render_pass.commands().len(), 3, "Old Frame 1 commands should be in z_state.render_pass");
    assert_eq!(graphics_command_buffer.commands().len(), 1, "Frame 2 commands should be in graphics_command_buffer");
    ...
    println!("Frame 2: After deferred expansion - {} commands, {} model matrices",
        graphics_command_buffer.commands().len(),
        z_state.model_matrices.len());
    ...
    assert_eq!(cmd2_0_mvp.0, 0, "Frame 2: First command should reference model matrix 0");
    assert_eq!(cmd2_1_mvp.0, 2, "Frame 2: Billboard should reference model matrix 2");

    // CRITICAL CHECK: Verify matrices are the NEW ones from Frame 2, not stale from Frame 1
    assert_eq!(z_state.model_matrices[0], model2_a, "Should have Frame 2 model_a, not Frame 1!");
    assert_eq!(z_state.model_matrices[2], billboard2, "Should have Frame 2 billboard, not Frame 1!");

    assert_ne!(z_state.model_matrices[0], model1_a, "Should NOT have Frame 1 model_a!");
