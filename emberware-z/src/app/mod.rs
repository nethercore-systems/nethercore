...
impl App {
    ...
    fn render_debug_panel(&mut self) {
        ...
        // If values changed and a callback is registered, invoke it
        if any_changed {
            if let Some(callback_ptr) = registry_clone.change_callback {
                // TODO: Invoke the WASM callback function
                tracing::debug!(
                    "Debug value changed, callback registered at ptr {}",
                    callback_ptr
                );
            }
        }
