...
fn material_mre(mut caller: Caller<'_, ZContext>, texture: u32) {
...
fn material_albedo(mut caller: Caller<'_, ZContext>, texture: u32) {
    let state = &mut caller.data_mut().ffi;
...
fn material_metallic(mut caller: Caller<'_, ZContext>, value: f32) {
    let state = &mut caller.data_mut().ffi;
...
fn material_roughness(mut caller: Caller<'_, ZContext>, value: f32) {
    let state = &mut caller.data_mut().ffi;
    let clamped = value.clamp(0.0, 1.0);
...
fn material_emissive(mut caller: Caller<'_, ZContext>, value: f32) {
    let state = &mut caller.data_mut().ffi;
...
fn material_shininess(caller: Caller<'_, ZContext>, value: f32) {
...
fn material_specular(mut caller: Caller<'_, ZContext>, color: u32) {
    let state = &mut caller.data_mut().ffi;
