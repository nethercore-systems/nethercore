...
fn push_identity(mut caller: Caller<'_, ZContext>) {
...
fn transform_set(mut caller: Caller<'_, ZContext>, matrix_ptr: u32) {
...
fn push_rotate_x(mut caller: Caller<'_, ZContext>, angle_deg: f32) {
    let state = &mut caller.data_mut().ffi;
    let current = state.current_model_matrix.unwrap_or_else(|| {
        state
            .model_matrices
...
///
/// Reads the current transform, applies rotation, and pushes the result.
fn push_rotate_y(mut caller: Caller<'_, ZContext>, angle_deg: f32) {
    let state = &mut caller.data_mut().ffi;
    let current = state.current_model_matrix.unwrap_or_else(|| {
        state
            .model_matrices
...
///
/// Reads the current transform, applies rotation, and pushes the result.
fn push_rotate_z(mut caller: Caller<'_, ZContext>, angle_deg: f32) {
    let state = &mut caller.data_mut().ffi;
    let current = state.current_model_matrix.unwrap_or_else(|| {
        state
            .model_matrices
...
///
/// Reads the current transform, applies scale, and pushes the result.
fn push_scale_uniform(mut caller: Caller<'_, ZContext>, s: f32) {
    let state = &mut caller.data_mut().ffi;
    let current = state.current_model_matrix.unwrap_or_else(|| {
        state
            .model_matrices
