...
- 8 additional presets still need to be designed, implemented, captured, reviewed, and fixed as needed, but expansion should stay blocked until at least one outdoor preset from the current 12 can pass the review bar cleanly. The indoor proof-of-life requirement is now closed by `Combat Lab` on run29.
...
- `Front Mass`
  Target: keep this as the first gating benchmark for weather-body work. The newest evidence says `ADVECT` alone was insufficient, `MASS` alone is also insufficient, and the right architecture is `MASS` as body plus `ADVECT` as subordinate motion. The occupancy/opacity pass, darker core-bias/support-washout pass, and now the `MASS/BANK` bank-profile pass are all closed, but the new bounds-authority split did improve the envelope. The next lever should therefore build weather-body ownership on top of the improved envelope, not reopen the flat-band cleanup theory or rerun duplicate capture.
- `Frozen Bed`
  Target: keep this as the second benchmark-gated surface track. The current floor identity is still too close to generic gloss/water, but the surrounding outdoor openness is now healthier after the bounds-authority split. The next lever should therefore focus on frozen-surface/material response on top of the improved envelope, not on retrying the same blank-field structural probes. Do not reopen `Frozen Tundra` showcase churn until the benchmark lane materially improves again.
- `Storm Front`
...
- `Combat Lab`
  Target: keep the run29 pass state, but lock future work to the corrected brief. Treat preset 10 as a room-integrated projection bay, not a HUD showcase. If revisited for polish, keep cadence on `GRID`, `VEIL/RAIN_WALL`, `LOBE`, and room-anchored emitters while using `PORTAL/RECT` only as a static frame.
