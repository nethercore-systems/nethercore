...
- `Front Mass`
  Target: keep this as the first gating benchmark for weather-body work. The newest evidence says `ADVECT` alone was insufficient, `MASS` alone is also insufficient, and the right architecture is `MASS` as body plus `ADVECT` as subordinate motion. The occupancy/opacity pass, darker core-bias/support-washout pass, `MASS/BANK` bank-profile pass, and now the `MASS/SHELF` local-body-hold pass are all closed, but the new bounds-authority split did improve the envelope. The next lever should therefore move beyond current `MASS` shelf shaping and aim at a more distinct front-body event on top of the improved envelope, not reopen the flat-band cleanup theory or rerun duplicate capture.
- `Frozen Bed`
  Target: keep this as the second benchmark-gated surface track. The current floor identity is still too close to generic gloss/water, but the surrounding outdoor openness is now healthier after the bounds-authority split. The new calmer `SURFACE/GLAZE` sheet path is now also closed, so the next lever should move beyond this exact glaze/coverage tweak and focus on stronger frozen-surface/material response on top of the improved envelope. Do not reopen `Frozen Tundra` showcase churn until the benchmark lane materially improves again.
- `Storm Front`
