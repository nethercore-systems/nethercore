...
pub fn execute(args: RunArgs) -> Result<()> {
    ...
    if args.p2p_test {
        return launch_p2p_test(&emberware_exe, &rom_path, &args);
    }
...
fn launch_p2p_test(emberware_exe: &PathBuf, rom_path: &PathBuf, args: &RunArgs) -> Result<()> {
    ...
    // Handle special "cargo:run" marker
    let is_cargo_run = emberware_exe.to_string_lossy() == "cargo:run";

    // Start player 2 in background
    let mut p2_cmd = if is_cargo_run {
        let mut cmd = Command::new("cargo");
        cmd.args(["run", "-p", "emberware-zx", "--"]);
        cmd.arg(rom_path);
        cmd
    } else {
        let mut cmd = Command::new(emberware_exe);
        cmd.arg(rom_path);
        cmd
    };
