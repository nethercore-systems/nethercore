//! Init command - create a new nether.toml manifest
//!
...
pub struct InitArgs {
    ...
    /// Overwrite existing nether.toml
    #[arg(long)]
    pub force: bool,
}
...
pub fn execute(args: InitArgs) -> Result<()> {
    ...
    let manifest_path = project_dir.join("nether.toml");

    // Check if manifest already exists
    if manifest_path.exists() && !args.force {
        anyhow::bail!(
            "nether.toml already exists at {}\nUse --force to overwrite",
    ...
    println!();
    println!("Next steps:");
    println!("  1. Edit nether.toml to customize your game metadata");
    println!("  2. Add render_mode() call in your game's init() function");
    println!("  3. Run 'nether build' to compile and pack your game");
...
fn generate_manifest(game_id: &str, game_title: &str, is_rust_project: bool) -> String {
    ...
    content.push_str("# Declare assets to bundle into the .nczx cartridge.\n");
    ...
    content.push_str("# path = \"assets/level.nczxmesh\"\n");
    content.push_str("#\n");
    ...
    content.push_str("# path = \"assets/walk.nczxanim\"\n");
    content.push_str("#\n");
...
mod tests {
    ...
    fn test_init_creates_file() {
        ...
        let manifest_path = dir.path().join("nether.toml");
