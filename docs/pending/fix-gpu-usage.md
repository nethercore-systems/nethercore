...
| File | Changes |
|------|---------|
| [core/src/app/event_loop.rs](core/src/app/event_loop.rs) | Add `next_frame_time()` to trait, change ControlFlow |
| [core/src/runtime.rs](core/src/runtime.rs) | Add `tick_duration()` getter |
| [emberware-z/src/app/mod.rs](emberware-z/src/app/mod.rs) | Implement smart redraw logic, track next frame time |

## Implementation Plan

### Step 1: Add `tick_duration()` getter to Runtime

In [core/src/runtime.rs](core/src/runtime.rs), add after line ~274:
...
In [core/src/app/event_loop.rs](core/src/app/event_loop.rs), add to the `ConsoleApp` trait:
...
In [core/src/app/event_loop.rs](core/src/app/event_loop.rs), replace lines 183-187:
...
In [core/src/app/event_loop.rs](core/src/app/event_loop.rs), line 116, remove:
```rust
...
In [emberware-z/src/app/mod.rs](emberware-z/src/app/mod.rs), add fields:
...
In [emberware-z/src/app/mod.rs](emberware-z/src/app/mod.rs), after line ~462 (the repaint_delay check), capture non-zero delays:
    ...
    match &self.mode {
        AppMode::Playing { .. } => {
            ...
            if let Some(runtime) = &self.runtime {
                let tick_duration = runtime.tick_duration();
                let next_tick = self.last_frame_time + tick_duration;
                // Also consider egui repaints
...
In [emberware-z/src/app/mod.rs](emberware-z/src/app/mod.rs) line ~657:
