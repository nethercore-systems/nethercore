...
//!
//! Simple IT (Impulse Tracker) music playback demo with split sample assets.
//! This version demonstrates explicit sample loading in nether.toml.
//!
//! Controls:
//! - A: Next song
//! - B: Previous song
...
const COLOR_ACID: u32 = 0x00FF88FF;
...
pub extern "C" fn render() {
    unsafe {
        ...
        draw_text_str(b"[Split Assets Mode]", 60.0, 100.0, 14.0, COLOR_GRAY);

        // Status
        let status = if IS_PAUSED { b"PAUSED" as &[u8] } else { b"PLAYING" };
        let status_color = if IS_PAUSED { COLOR_PAUSED } else { COLOR_PLAYING };
        draw_text_str(status, 60.0, 130.0, 20.0, status_color);

        // Position
        draw_text_str(b"Order:", 60.0, 180.0, 16.0, COLOR_GRAY);
        let order_text = format_2digit(order);
        set_color(COLOR_WHITE);
        draw_text(order_text.as_ptr(), 2, 130.0, 180.0, 16.0);

        draw_text_str(b"Row:", 180.0, 180.0, 16.0, COLOR_GRAY);
        let row_text = format_2digit(row);
        set_color(COLOR_WHITE);
        draw_text(row_text.as_ptr(), 2, 230.0, 180.0, 16.0);

        // Tempo and Volume
        draw_text_str(b"Tempo:", 60.0, 220.0, 16.0, COLOR_GRAY);
        let tempo_text = format_3digit(CURRENT_TEMPO);
        set_color(COLOR_WHITE);
        draw_text(tempo_text.as_ptr(), 3, 130.0, 220.0, 16.0);

        draw_text_str(b"Volume:", 200.0, 220.0, 16.0, COLOR_GRAY);
        let vol_pct = (VOLUME * 100.0) as u32;
        let vol_text = format_3digit(vol_pct);
        set_color(COLOR_WHITE);
        draw_text(vol_text.as_ptr(), 3, 280.0, 220.0, 16.0);
        draw_text_str(b"%", 310.0, 220.0, 16.0, COLOR_GRAY);

        // Controls
        draw_text_str(b"[A/B] Song  [X] Pause  [Y] Restart  [Up/Dn] Tempo  [L/R] Vol",
            60.0, 300.0, 14.0, COLOR_GRAY);
    }
}
