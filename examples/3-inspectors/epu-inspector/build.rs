//! Build script for epu-inspector
//!
//! Generates Rust code from WGSL metadata blocks for the EPU Inspector tool.
//!
//! This script:
//! 1. Finds all WGSL opcode files in `nethercore-zx/shaders/epu/bounds/` and `features/`
//! 2. Parses the `@epu_meta_begin`/`@epu_meta_end` blocks from each file
//! 3. Generates `epu_meta_gen.rs` with const/static tables for runtime use

// Include the parser module from src/
...
use wgsl_meta_parser::{parse_wgsl_meta, MapKind, OpcodeMeta, OpcodeKind};

/// Maximum number of opcodes supported (5-bit opcode = 0x00..0x1F)
const MAX_OPCODES: usize = 32;
/// Maximum number of variants per opcode
const MAX_VARIANTS: usize = 8;
/// Maximum number of domains per opcode
const MAX_DOMAINS: usize = 4;
/// Maximum number of fields per opcode
const MAX_FIELDS: usize = 8;

fn main() {
    // Tell Cargo to rerun this script if any WGSL files change
    let shader_base = PathBuf::from("../../../nethercore-zx/shaders/epu");
        ...
        if meta.fields.len() > MAX_FIELDS {
            panic!(
                "Too many fields ({}) for opcode 0x{:02X} ({}) in {}\n\
                 Maximum allowed: {}",
                meta.fields.len(), meta.opcode, meta.name, path_str, MAX_FIELDS
            );
        }
    ...
    let out_path = PathBuf::from(&out_dir).join("epu_meta_gen.rs");
    ...
    println!("cargo:warning=Generated {} with {} opcodes", out_path.display(), opcodes.len());
}
...
fn generate_code(opcodes: &BTreeMap<u8, OpcodeMeta>) -> String {
    ...
    // Count
    code.push_str(&format!("/// Total number of defined opcodes\n"));
    code.push_str(&format!("pub const OPCODE_COUNT: usize = {};\n\n", opcodes.len()));

    // Maximum opcode value
    let max_opcode = opcodes.keys().max().copied().unwrap_or(0) as usize;
    code.push_str(&format!("/// Highest opcode number (for array sizing)\n"));
    code.push_str(&format!("pub const MAX_OPCODE: usize = {};\n\n", max_opcode));
    ...
    code.push_str(&format!("/// Opcode info indexed by opcode number (None for undefined opcodes)\n"));
    code.push_str(&format!("pub static OPCODES: [Option<OpcodeInfo>; {}] = [\n", MAX_OPCODES));
    ...
    code.push_str(&format!("/// Variant names per opcode (up to {})\n", MAX_VARIANTS));
    code.push_str(&format!("pub static VARIANTS: [&[&str]; {}] = [\n", MAX_OPCODES));
    for i in 0..MAX_OPCODES {
        if let Some(meta) = opcodes.get(&(i as u8)) {
            if meta.variants.is_empty() {
                code.push_str("    &[],\n");
            } else {
                let variants_str: Vec<String> = meta.variants.iter()
                    .map(|v| format!("\"{}\"", v))
                    .collect();
                code.push_str(&format!("    &[{}],\n", variants_str.join(", ")));
            }
    ...
    code.push_str(&format!("/// Domain names per opcode (up to {})\n", MAX_DOMAINS));
    code.push_str(&format!("pub static DOMAINS: [&[&str]; {}] = [\n", MAX_OPCODES));
    for i in 0..MAX_OPCODES {
        if let Some(meta) = opcodes.get(&(i as u8)) {
            if meta.domains.is_empty() {
                code.push_str("    &[],\n");
            } else {
                let domains_str: Vec<String> = meta.domains.iter()
                    .map(|d| format!("\"{}\"", d))
                    .collect();
                code.push_str(&format!("    &[{}],\n", domains_str.join(", ")));
            }
    ...
    code.push_str(&format!("pub static FIELD_SPECS: [&[FieldSpec]; {}] = [\n", MAX_OPCODES));
    for i in 0..MAX_OPCODES {
        if let Some(meta) = opcodes.get(&(i as u8)) {
            if meta.fields.is_empty() {
                code.push_str("    &[],\n");
