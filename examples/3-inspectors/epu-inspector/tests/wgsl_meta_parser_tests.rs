...
fn test_parse_valid_plane_meta() {
    ...
    let intensity = meta.fields.iter().find(|f| f.field_name == "intensity").unwrap();
    ...
    let param_a = meta.fields.iter().find(|f| f.field_name == "param_a").unwrap();
    ...
    let direction = meta.fields.iter().find(|f| f.field_name == "direction").unwrap();
    assert_eq!(direction.label, "normal");
    assert_eq!(direction.map, MapKind::Dir16Oct);

    let alpha_a = meta.fields.iter().find(|f| f.field_name == "alpha_a").unwrap();
