...
// Opcodes (v2 spec: NOP=0; bounds=1..7; features=8..)
...
const PRESET_SUNNY_MEADOW: [[u64; 2]; 8] = [
    ...
    // FLOW: clouds (LERP)
    [hi(OP_FLOW, REGION_SKY, BLEND_LERP, 0, 0xFFFFFF, 0xE0E8F0), lo(60, 32, 96, 0x30, 64, DIR_UP, 15, 8)],
    NOP_LAYER,
    NOP_LAYER,
];
...
const PRESET_CYBERPUNK_ALLEY: [[u64; 2]; 8] = [
    ...
    // LOBE: magenta glow
    [hi(OP_LOBE, REGION_ALL, BLEND_ADD, 12, 0xFF00FF, 0xFF44AA), lo(140, 24, 0, 2, 180, 0x30C0, 15, 10)],
    // LOBE: cyan glow
    [hi(OP_LOBE, REGION_ALL, BLEND_ADD, 10, 0x00FFFF, 0x00AAFF), lo(120, 28, 0, 2, 170, 0xD060, 15, 10)],
    // FOG: purple haze
...
const PRESET_VOID_STARS: [[u64; 2]; 8] = [
    ...
    [hi(OP_SCATTER, REGION_ALL, BLEND_ADD, 12, 0x8888FF, 0x4444AA), lo(180, 255, 8, 0x44, 0, 0, 12, 8)],
    NOP_LAYER,
    NOP_LAYER,
];
...
const PRESET_UNDERWATER_CAVE: [[u64; 2]; 8] = [
    ...
    NOP_LAYER,
    // FLOW: caustics
    [hi(OP_FLOW, REGION_ALL, BLEND_ADD, 4, 0x40C0C0, 0x208080), lo(80, 40, 90, 0x22, 120, 0x9870, 15, 10)],
    // SCATTER: bubbles
    [hi(OP_SCATTER, REGION_ALL, BLEND_ADD, 6, 0xFFFFFF, 0xAADDFF), lo(140, 80, 12, 0x64, 0, 0, 15, 12)],
    NOP_LAYER,
    NOP_LAYER,
];
...
const PRESET_STORM_FRONT: [[u64; 2]; 8] = [
    ...
    [hi(OP_LOBE, REGION_ALL, BLEND_ADD, 4, 0x405060, 0x304050), lo(60, 24, 0, 1, 40, DIR_UP, 12, 10)],
    // BAND: dark cloud band
    [hi(OP_BAND, REGION_SKY, BLEND_MULTIPLY, 0, 0x101820, 0x081018), lo(120, 60, 140, 0, 80, DIR_UP, 15, 12)],
    // FOG: mist
...
const PRESET_NEON_CITY: [[u64; 2]; 8] = [
    ...
    [hi(OP_LOBE, REGION_ALL, BLEND_ADD, 8, 0xFF8844, 0xCC6622), lo(100, 20, 0, 2, 120, 0x8000, 15, 12)],
    // BAND: sky glow pollution
    [hi(OP_BAND, REGION_SKY, BLEND_SCREEN, 2, 0x442244, 0x221122), lo(40, 80, 180, 0, 40, DIR_UP, 10, 8)],
    NOP_LAYER,
    // GRID: building windows
    [hi(OP_GRID, REGION_WALLS, BLEND_ADD, 6, 0xFFCC88, 0xFF8844), lo(120, 32, 20, 0x02, 0, 0, 15, 12)],
    // SCATTER: neon signs
...
const PRESET_ENCHANTED_FOREST: [[u64; 2]; 8] = [
    ...
    [hi(OP_FLOW, REGION_ALL, BLEND_ADD, 2, 0xFFFFAA, 0xAAAA66), lo(40, 20, 80, 0x30, 120, 0x6080, 10, 6)],
    NOP_LAYER,
];
...
const PRESET_ARCTIC_TUNDRA: [[u64; 2]; 8] = [
    ...
    [hi(OP_RAMP, REGION_ALL, BLEND_ADD, 0, 0x6080A8, 0x90B8E8), lo(180, 210, 220, 235, 0xBE, DIR_UP, 15, 15)],
    // LOBE: cold sun
    [hi(OP_LOBE, REGION_ALL, BLEND_ADD, 8, 0xE8F4FF, 0xC0D8FF), lo(100, 16, 0, 0, 0, 0xC0A0, 15, 12)],
    // FOG: cold haze
    ...
    // FLOW: aurora
    [hi(OP_FLOW, REGION_SKY, BLEND_SCREEN, 0, 0x40FFB0, 0x8040FF), lo(20, 24, 60, 0x30, 160, 0x5090, 12, 8)],
    NOP_LAYER,
];
...
const PRESET_NEON_ARCADE: [[u64; 2]; 8] = [
    ...
    [hi(OP_BAND, REGION_ALL, BLEND_ADD, 10, 0xFF00FF, 0xAA00AA), lo(200, 48, 96, 0, 80, DIR_UP, 15, 12)],
    // BAND: cyan stripes offset
    [hi(OP_BAND, REGION_ALL, BLEND_ADD, 8, 0x00FFFF, 0x00AAAA), lo(160, 48, 144, 0, 120, DIR_UP, 15, 12)],
    NOP_LAYER,
    // GRID: cyan floor tiles
    [hi(OP_GRID, REGION_FLOOR, BLEND_ADD, 8, 0x00FFFF, 0x008888), lo(160, 64, 16, 0x14, 0, 0, 15, 12)],
...
const PRESET_DESERT_DUNES: [[u64; 2]; 8] = [
    ...
    // FLOW: sand drift
    [hi(OP_FLOW, REGION_FLOOR, BLEND_LERP, 0, 0xD0A050, 0xB08040), lo(80, 24, 120, 0x11, 90, DIR_UP, 12, 8)],
    NOP_LAYER,
    NOP_LAYER,
];
...
const PRESET_ALIEN_PLANET: [[u64; 2]; 8] = [
    ...
    [hi(OP_LOBE, REGION_ALL, BLEND_ADD, 12, 0xFF8800, 0x0088FF), lo(180, 20, 0, 1, 60, 0xA0C0, 15, 12)],
    ...
    // DECAL: alien sun
    [hi(OP_DECAL, REGION_SKY, BLEND_ADD, 14, 0xFF6600, 0x0066FF), lo(240, 0x04, 20, 0, 40, 0xA0C0, 15, 15)],
    // FLOW: gas currents
    [hi(OP_FLOW, REGION_ALL, BLEND_LERP, 0, 0x804080, 0x408040), lo(50, 40, 100, 0x22, 160, 0x5090, 10, 8)],
    NOP_LAYER,
];
...
const PRESET_STORMY_OCEAN: [[u64; 2]; 8] = [
    ...
    // BAND: horizon line
    [hi(OP_BAND, REGION_ALL, BLEND_LERP, 0, 0x405060, 0x304050), lo(80, 40, 128, 0, 40, DIR_UP, 12, 10)],
    // FOG: sea spray
    [hi(OP_FOG, REGION_ALL, BLEND_SCREEN, 0, 0x405060, 0x304050), lo(100, 120, 80, 0, 0, DIR_UP, 10, 8)],
    // FLOW: waves
    [hi(OP_FLOW, REGION_FLOOR, BLEND_ADD, 4, 0x6080A0, 0x406080), lo(100, 60, 80, 0x21, 80, 0x80A0, 15, 10)],
    // SCATTER: foam
...
const PRESET_CRYSTAL_CAVERN: [[u64; 2]; 8] = [
    ...
    [hi(OP_LOBE, REGION_ALL, BLEND_ADD, 10, 0x8040FF, 0x4020AA), lo(140, 18, 0, 1, 60, 0x8010, 15, 12)],
    // FOG: faint mist
    ...
    [hi(OP_GRID, REGION_WALLS, BLEND_ADD, 6, 0x6040A0, 0x402080), lo(80, 24, 12, 0x22, 48, 0, 12, 10)],
    NOP_LAYER,
];
...
const PRESET_TOXIC_WASTELAND: [[u64; 2]; 8] = [
    ...
    // LOBE: toxic glow
    [hi(OP_LOBE, REGION_ALL, BLEND_ADD, 8, 0x80FF00, 0x40AA00), lo(120, 16, 0, 2, 140, 0x9060, 15, 12)],
    // FOG: toxic haze
    ...
    // FLOW: toxic sludge
    [hi(OP_FLOW, REGION_FLOOR, BLEND_ADD, 6, 0x80C000, 0x408000), lo(120, 30, 90, 0x20, 150, DIR_UP, 12, 10)],
    // GRID: industrial grating
    [hi(OP_GRID, REGION_WALLS, BLEND_ADD, 2, 0x404040, 0x303030), lo(40, 32, 8, 0x11, 0, 0, 10, 8)],
    NOP_LAYER,
];
...
const PRESET_CHERRY_BLOSSOM: [[u64; 2]; 8] = [
    ...
    [hi(OP_FLOW, REGION_ALL, BLEND_LERP, 0, 0xFFCCDD, 0xFFAABB), lo(60, 30, 80, 0x21, 120, 0x40B0, 12, 10)],
    // DECAL: sun through branches
    [hi(OP_DECAL, REGION_SKY, BLEND_ADD, 10, 0xFFFFE0, 0xFFF0C0), lo(180, 0x02, 16, 0, 30, DIR_SUN, 15, 12)],
    NOP_LAYER,
];
...
const PRESET_VOID_DIMENSION: [[u64; 2]; 8] = [
    ...
    // LOBE: eerie glow
    [hi(OP_LOBE, REGION_ALL, BLEND_ADD, 6, 0x8000FF, 0x400088), lo(80, 12, 0, 2, 120, 0x70C0, 15, 10)],
    // BAND: dimensional rift
    [hi(OP_BAND, REGION_ALL, BLEND_ADD, 12, 0xFF00FF, 0x0000FF), lo(160, 30, 128, 80, 160, 0x5090, 15, 15)],
    // FOG: void mist
    ...
    // GRID: reality grid
    [hi(OP_GRID, REGION_ALL, BLEND_ADD, 4, 0x400880, 0x200440), lo(60, 48, 8, 0x14, 64, 0, 10, 8)],
    NOP_LAYER,
    NOP_LAYER,
];
...
const PRESET_RETRO_SYNTHWAVE: [[u64; 2]; 8] = [
    ...
    [hi(OP_LOBE, REGION_ALL, BLEND_ADD, 10, 0xFF4488, 0xFF0044), lo(200, 28, 0, 1, 40, 0xF020, 15, 12)],
    // BAND: sun stripe
    [hi(OP_BAND, REGION_SKY, BLEND_ADD, 14, 0xFF8844, 0xFF4400), lo(220, 20, 140, 80, 40, 0xF020, 15, 15)],
    NOP_LAYER,
    // GRID: perspective floor grid
    [hi(OP_GRID, REGION_FLOOR, BLEND_ADD, 8, 0x00FFFF, 0x0088AA), lo(180, 64, 16, 0x16, 0, 0, 15, 12)],
    // DECAL: setting sun
    [hi(OP_DECAL, REGION_SKY, BLEND_ADD, 15, 0xFF6600, 0xFF0066), lo(255, 0x04, 24, 0, 20, 0xF020, 15, 15)],
    // SCATTER: stars
    [hi(OP_SCATTER, REGION_SKY, BLEND_ADD, 6, 0xFFFFFF, 0xAABBFF), lo(160, 100, 12, 0x43, 0, 0, 12, 10)],
    NOP_LAYER,
];
...
const PRESET_ANCIENT_TEMPLE: [[u64; 2]; 8] = [
    ...
    // LOBE: torch glow
    [hi(OP_LOBE, REGION_ALL, BLEND_ADD, 10, 0xFF8840, 0xCC6620), lo(140, 16, 0, 2, 200, 0x60A0, 15, 12)],
    // FOG: dust and smoke
    ...
    [hi(OP_SCATTER, REGION_ALL, BLEND_ADD, 2, 0xCCBBAA, 0xAA9988), lo(80, 40, 10, 0x32, 0, 0, 10, 8)],
    // FLOW: torch smoke
    [hi(OP_FLOW, REGION_ALL, BLEND_SCREEN, 0, 0x403020, 0x201810), lo(30, 40, 60, 0x20, 120, DIR_UP, 10, 8)],
];
...
const PRESET_DEEP_SPACE_NEBULA: [[u64; 2]; 8] = [
    ...
    [hi(OP_LOBE, REGION_ALL, BLEND_SCREEN, 6, 0x8040C0, 0x402080), lo(100, 32, 0, 1, 40, 0x50A0, 15, 10)],
    // LOBE: nebula glow (teal)
    [hi(OP_LOBE, REGION_ALL, BLEND_SCREEN, 5, 0x208080, 0x104040), lo(80, 28, 0, 1, 35, 0xB060, 15, 10)],
    // FOG: cosmic dust
    ...
    // FLOW: gas currents
    [hi(OP_FLOW, REGION_ALL, BLEND_SCREEN, 2, 0x604080, 0x408060), lo(40, 50, 110, 0x20, 160, 0x6090, 8, 6)],
    NOP_LAYER,
];
